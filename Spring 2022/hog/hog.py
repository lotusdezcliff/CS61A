...
# python3 ok -q [question_number] --local  (test code)
# python3 ok -q [question_number] -u --local (unlock pre-coding question)
...
def roll_dice(num_rolls, dice=six_sided):
    ...
    if exist_one:
        ...
        return result
    # END PROBLEM 1
#Test summary 59 test cases passed! No cases failed.
...
def hefty_hogs(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    #Roll no die
    opponent_score_record = opponent_score
    if  opponent_score == 0:
        return 1
    else:
        while opponent_score_record >= 1:
            player_score = digit_fn(opponent_score_record % 10)(player_score) 
            opponent_score_record = (opponent_score_record - (opponent_score_record % 10)) // 10
    return player_score % 30
    # END PROBLEM 2
#     38 test cases passed! No cases failed.
...
def hog_pile(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    if str(player_score)[len(str(player_score)) - 1] == str(opponent_score)[len(str(opponent_score)) - 1]:
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            score0 += take_turn(num_dice, score0, score1, dice, goal)
            score0 += hog_pile(score0, score1)
            say(score0, score1, leader)
        else:
            num_dice = strategy1(score1, score0)
            score1 += take_turn(num_dice, score1, score0, dice, goal)
            score1 += hog_pile(score1, score0)
            say(score1, score0, leader)
        leader, message = say(score0, score1, leader)
        if message and message != "":
            print(message)
        who = next_player(who)
    # END PROBLEM 5
    #    110 test cases passed! No cases failed.
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 == score1:
        ...
        diff = score0 - score1
        return 0, f"Player 0 takes the lead by {diff}"
    elif score0 < score1 and last_leader != 1:
        diff = score1 - score0
        return 1, f"Player 1 takes the lead by {diff}"
    elif score0 > score1 and last_leader == 0:
        return 0, None
    elif score0 < score1 and last_leader == 1:
        return 1, None
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    result, max_result, max_roll = 0, 0, 0
    for i in range(1, 11):
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    sus_pnts = hog_pile(score, opponent_score)
